...
func TestReplayPreventer(t *testing.T) {
	...
	err = rp.Check(h1[0], 2, 2)
	assert.Error(t, err)
	err = rp.Check(h1[1], 2, 2)
	assert.Error(t, err)
	err = rp.Check(h1[2], 2, 2)
	assert.Error(t, err)
	err = rp.Check(h2[0], 2, 2)
	assert.NoError(t, err)
	err = rp.Check(h2[1], 2, 2)
	assert.NoError(t, err)
	err = rp.Check(h2[2], 2, 2)
	assert.NoError(t, err)
	...
	err = rp.Check(h1[0], 3, 3)
	assert.Error(t, err)
	err = rp.Check(h1[1], 3, 3)
	assert.Error(t, err)
	err = rp.Check(h1[2], 3, 3)
	assert.Error(t, err)
	err = rp.Check(h2[0], 3, 3)
	assert.Error(t, err)
	err = rp.Check(h2[1], 3, 3)
	assert.Error(t, err)
	err = rp.Check(h2[2], 3, 3)
	assert.Error(t, err)
	err = rp.Check(h3[0], 3, 3)
	assert.NoError(t, err)
	err = rp.Check(h3[1], 3, 3)
	assert.NoError(t, err)
	err = rp.Check(h3[2], 3, 3)
	assert.NoError(t, err)
	...
	err = rp.Check(h1[0], 4, 4)
	assert.NoError(t, err)
	err = rp.Check(h1[1], 4, 4)
	assert.NoError(t, err)
	err = rp.Check(h1[2], 4, 4)
	assert.NoError(t, err)
	err = rp.Check(h2[0], 4, 4)
	assert.Error(t, err)
	err = rp.Check(h2[1], 4, 4)
	assert.Error(t, err)
	err = rp.Check(h2[2], 4, 4)
	assert.Error(t, err)
	err = rp.Check(h3[0], 4, 4)
	assert.Error(t, err)
	err = rp.Check(h3[1], 4, 4)
	assert.Error(t, err)
	err = rp.Check(h3[2], 4, 4)
	assert.Error(t, err)
	...
	err = rp.Check(h4[0], 4, 4)
	assert.NoError(t, err)
	err = rp.Check(h4[1], 4, 4)
	assert.NoError(t, err)
	err = rp.Check(h4[2], 4, 4)
	assert.NoError(t, err)
