...
func TestFillGenesisState(t *testing.T) {
	...
	assert.Equal(t, new(types.Currency).Set(10), s.GetBalance(addr1, fromStage))
	...
	assert.Equal(t, new(types.Currency).Set(0), s.GetBalance(addr1, fromStage))
	assert.Equal(t, new(types.Currency).Set(0), s.GetBalance(addr2, fromStage))

	// check if the genesis state is filled correctly
	addr0, _ := hex.DecodeString(addr0Json)
	assert.Equal(t, new(types.Currency).Set(100), s.GetBalance(addr0, fromStage))
	...
	err = FillGenesisState(s, genState)
	assert.NoError(t, err)
	stake := s.GetStake(addr0, fromStage)
	...
	assert.Equal(t, addr0, s.GetHolderByValidator(valAddr, fromStage))
}
