...
func TestTxIndexer(t *testing.T) {
	...
	txs := s.GetTxIndexerHeight(1)
	assert.Nil(t, txs)

	txs = s.GetTxIndexerHeight(2)
	assert.Nil(t, txs)
	...
	// GetHash
	height := s.GetTxIndexerHash(h1[0])
	assert.Equal(t, int64(1), height)

	height = s.GetTxIndexerHash(h1[1])
	assert.Equal(t, int64(1), height)

	height = s.GetTxIndexerHash(h1[2])
	assert.Equal(t, int64(1), height)

	height = s.GetTxIndexerHash(h2[0])
	assert.Equal(t, int64(2), height)

	height = s.GetTxIndexerHash(h2[1])
	assert.Equal(t, int64(2), height)

	height = s.GetTxIndexerHash(h2[2])
	assert.Equal(t, int64(2), height)
