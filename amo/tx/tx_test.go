...
func TestValidWithdraw(t *testing.T) {
	...
	rc, _, _ = t1.Execute(s)
	assert.Equal(t, code.TxCodeOK, rc)
	assert.Equal(t, new(types.Currency).Set(1000), &s.GetStake(alice.addr, fromStage).Amount)
	...
	rc, _, _ = t1.Execute(s)
	assert.Equal(t, code.TxCodeOK, rc)
	assert.Nil(t, s.GetStake(alice.addr, fromStage))
	assert.NotNil(t, s.GetStake(bob.addr, fromStage))
}
...
func TestValidDelegate(t *testing.T) {
	...
	rc, _, _ = t1.Execute(s)
	assert.Equal(t, code.TxCodeOK, rc)
	assert.Equal(t, new(types.Currency).Set(1000), &s.GetDelegate(bob.addr, fromStage).Amount)
}
...
func TestValidRetract(t *testing.T) {
	...
	rc, _, _ = t1.Execute(s)
	assert.Equal(t, code.TxCodeOK, rc)
	assert.Equal(t, new(types.Currency).Set(100), &s.GetDelegate(bob.addr, fromStage).Amount)
	...
	rc, _, _ = t1.Execute(s)
	assert.Equal(t, code.TxCodeOK, rc)
	assert.Nil(t, s.GetDelegate(bob.addr, fromStage))

	assert.Equal(t, new(types.Currency).Set(2000), &s.GetStake(alice.addr, fromStage).Amount)
}
...
func TestStakeLockup(t *testing.T) {
	...
	stake := s.GetStake(makeTestAddress("alice"), fromStage)
